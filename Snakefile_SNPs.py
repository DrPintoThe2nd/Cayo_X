...
rule gatk_genotypegvcf_chrx:
	...
	shell:
		"""
		gatk --java-options "-Xmx4g" GenotypeGVCFs -R {params.genome} -V {input.gvcf} -O {output}
		"""
...
rule hard_filter:
	...
	shell:
		"""
		gatk SelectVariants -R {params.genome} -V {input} -O {output} --restrict-alleles-to BIALLELIC -select "AN >= 4 && MQ > 40.0 && QD > 5.0 && DP >= 10.0 && DP <= 1000.0" 
		"""
...
rule hard_filter_autosomes:
	...
	shell:
		"""
		gatk SelectVariants -R {params.genome} -V {input} -O {output} --restrict-alleles-to BIALLELIC -select "AN >= 4 && MQ > 40.0 && QD > 7.0 && DP >= 10.0 && DP <= 1000.0" 
		"""
