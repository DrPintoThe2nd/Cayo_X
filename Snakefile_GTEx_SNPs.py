...
#gatk_combinegvcfs_chrx rule
		"merged_vcfs/GRCh38_XX.chrx.gatk.combineVCF.vcf.gz",
#gatk_genotypegvcf_chrx rule 
		"merged_vcfs/GRCh38_XX.chrx.gatk.called.raw.vcf.gz",		
#hard_filter rule
		"merged_vcfs/GRCh38_XX.chrx.gatk.called.hard.filter.vcf.gz",
#subset_individuals_hard_filter rule
		expand("merged_vcfs/GRCh38_XX.chrx.gatk.called.hard.filter.{sample}.vcf.gz", sample=dna_samples),
		expand("merged_vcfs/GRCh38_XX.chrx.gatk.called.hard.filter.{sample}.vcf.gz.tbi", sample=dna_samples),
#gatk_selectheterozygous rule
		expand("Xchr_vcfs/GRCh38_XX.chrx.gatk.called.hard.filter.het.{sample}.vcf.gz", sample=dna_samples),
...
#gatk_combinegvcfs_autosomes rule
		expand("merged_vcfs/GRCh38.chr{chr_n}.gatk.combinegvcf.g.vcf.gz", chr_n=config["autosomes"]),
#gatk_genotypegvcf_autosomes rule
		expand("merged_vcfs/GRCh38.chr{chr_n}.gatk.called.raw.vcf.gz", chr_n=config["autosomes"]),
#hard_filter_autosomes rule
		expand("merged_vcfs/GRCh38.chr{chr_n}.gatk.called.hard.filter.vcf.gz", chr_n=config["autosomes"]),
		expand("merged_vcfs/GRCh38.chr{chr_n}.gatk.called.hard.filter.vcf.gz.tbi", chr_n=config["autosomes"]),
#subset_individuals_hard_filter_autosomes rule
		expand("merged_vcfs/GRCh38.chr{chr_n}.gatk.called.hard.filter.{sample}.vcf.gz.tbi", sample=dna_samples, chr_n=config["autosomes"]),
#gatk_selectheterozygous_autosomes rule
		expand("autosomes_vcfs/GRCh38.chr{chr_n}.gatk.called.hard.filter.het.{sample}.vcf.gz", sample=dna_samples, chr_n=config["autosomes"]),
...
rule hisat2_map_reads:
	...
	shell:
		"""
		hisat2 -p {params.threads} --dta --rg-id {params.id} --rg SM:{params.sm} --rg LB:{params.lb} --rg PU:{params.pu} --rg PL:{params.pl} -x GRCh38/reference/xyalign_noY.masked -1 {input.fq1} -2 {input.fq2} | samtools sort -@4 -O bam - -o {output};
		"""
...
rule stats_rna:
	...
	shell:
		"""
		samtools stats -@2 {input} | grep '^SN' | cut -f 2- > {output};
		"""
...
rule gatk_genotypegvcf_chrx:
	input:
		gvcf = "merged_vcfs/GRCh38_XX.chrx.gatk.combineVCF.vcf.gz",
	output:
		vcf = "merged_vcfs/GRCh38_XX.chrx.gatk.called.raw.vcf.gz",
	params:
		genome = genome,
	threads:
		4
...
rule hard_filter:
	input:
		"merged_vcfs/GRCh38_XX.chrx.gatk.called.raw.vcf.gz",
	output:
		vcf = "merged_vcfs/GRCh38_XX.chrx.gatk.called.hard.filter.vcf.gz",
		idx = "merged_vcfs/GRCh38_XX.chrx.gatk.called.hard.filter.vcf.gz.tbi",
	params:
		genome = genome,
	shell:
		"""
		gatk SelectVariants -R {params.genome} -V {input} -O {output.vcf} --select-type-to-include SNP --restrict-alleles-to BIALLELIC -select "AN >= 4 && MQ > 40.0 && QD > 5.0 && DP >= 10.0 && DP <= 1000.0";
		touch -c {output.idx};
		"""
...
rule subset_individuals_hard_filter:
	input:
		"merged_vcfs/GRCh38_XX.chrx.gatk.called.hard.filter.vcf.gz",
	output:
		vcf = "merged_vcfs/GRCh38_XX.chrx.gatk.called.hard.filter.{sample}.vcf.gz",
		index = "merged_vcfs/GRCh38_XX.chrx.gatk.called.hard.filter.{sample}.vcf.gz.tbi",
	params:
...
rule gatk_selectheterozygous:
	input:
		"merged_vcfs/GRCh38_XX.chrx.gatk.called.hard.filter.{sample}.vcf.gz",
	output:
		"Xchr_vcfs/GRCh38_XX.chrx.gatk.called.hard.filter.het.{sample}.vcf.gz",
	params:
		genome = genome,
	shell:
		"""
...
rule gatk_combinegvcfs_autosomes:
	...
	output:
		"merged_vcfs/GRCh38.chr{chr_n}.gatk.combinegvcf.g.vcf.gz",
	params:
		genome = genome,
		chr_n = "{chr_n}",
	threads:
...
rule gatk_genotypegvcf_autosomes:
	input:
		expand("merged_vcfs/GRCh38.chr{{chr_n}}.gatk.combinegvcf.g.vcf.gz", chr_n=config["autosomes"]),
	output:
		"merged_vcfs/GRCh38.chr{chr_n}.gatk.called.raw.vcf.gz",
	params:
		genome = genome,
	threads:
		4
...
rule hard_filter_autosomes:
	input:
		"merged_vcfs/GRCh38.chr{chr_n}.gatk.called.raw.vcf.gz"
	output:
		vcf = "merged_vcfs/GRCh38.chr{chr_n}.gatk.called.hard.filter.vcf.gz",
		idx = "merged_vcfs/GRCh38.chr{chr_n}.gatk.called.hard.filter.vcf.gz.tbi",
	params:
		genome = genome,
	shell:
		"""
		gatk SelectVariants -R {params.genome} -V {input} -O {output.vcf} --select-type-to-include SNP --restrict-alleles-to BIALLELIC -select "AN >= 4 && MQ > 40.0 && QD > 7.0 && DP >= 10.0 && DP <= 1000.0";
		touch -c {output.idx};
		"""

rule subset_individuals_hard_filter_autosomes:
	input:
		"merged_vcfs/GRCh38.chr{chr_n}.gatk.called.hard.filter.vcf.gz",
	output:
		vcf = "merged_vcfs/GRCh38.chr{chr_n}.gatk.called.hard.filter.{sample}.vcf.gz",
		index = "merged_vcfs/GRCh38.chr{chr_n}.gatk.called.hard.filter.{sample}.vcf.gz.tbi",
	params:
		sample = lambda wildcards: config[wildcards.sample]["SM"],
	shell:
		"""
...
rule gatk_selectheterozygous_autosomes:
	input:
		"merged_vcfs/GRCh38.chr{chr_n}.gatk.called.hard.filter.{sample}.vcf.gz", 
	output:
		"autosomes_vcfs/GRCh38.chr{chr_n}.gatk.called.hard.filter.het.{sample}.vcf.gz",
	params:
		genome = genome,
	shell:
		"""
